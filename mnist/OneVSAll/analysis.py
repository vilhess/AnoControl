...
normal = st.sidebar.slider("Normal digit: ", min_value=0, max_value=9, value=0, step=1)
threshold = st.sidebar.slider("Threshold: ", min_value=0., max_value=0.2, step=0.01, value=0.05)

deepsvd_err = Image.open(f"results/figures/deepsvdd/mean_scores_{normal}.jpg")
vae_err = Image.open(f"results/figures/vae/mean_scores_{normal}.jpg")
cvae_err = Image.open(f"results/figures/cvae/mean_scores_{normal}.jpg")
fanogan_err = Image.open(f"results/figures/fanogan/mean_scores_{normal}.jpg")
...
with open(f"results/p_values/deepsvdd/pval_{normal}.json", "r") as file:
    p_values_deepsvdd = json.load(file)

with open(f"results/p_values/vae/pval_{normal}.json", "r") as file:
    p_values_vae = json.load(file)

with open(f"results/p_values/cvae/pval_{normal}.json", "r") as file:
    p_values_cvae = json.load(file)

with open(f"results/p_values/fanogan/pval_{normal}.json", "r") as file:
...
for digit in range(10):
    ...
    results.append({
        "Digit": digit,
        "Normal": "Yes" if digit == normal else "No",
        "Threshold": threshold,
